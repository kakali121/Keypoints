...
INTERVAL = 60 # 150 frames per inverval
...
def analyze_kpt_des(frame, keypoints, descriptors, filename, video):
    ...
    # Read the keypoints and descriptors from the file
    while k > 0:
        k -= 1
        # Check if is the first frame
        if k == INTERVAL - 1:
        ...
        # n = int(0.9*len(matches))
        # matches = matches[:n]
        matches = [m for m in matches if m.distance < MAX_MATCH_DISTANCE]

        # Create a list of keypoints from the matching descriptors
        kpts_cur_temp = []
        des_cur_temp = []
        kpts_fin_temp = []
        des_fin_temp = []
        ...
        out.write(frame[k])
        if k == 0:
            print("Reference:", hull1.area, "Current:", hull2.area)
...
if __name__ == "__main__":
    ...
    for i in range(int(len(frames)/INTERVAL)):
        print("Interval", i)
        analyze_kpt_des(frames, frame_kpt, frame_des, "demo_kpt_des/demo_kpt_des_%d.yml"%(i+1), "demo_kpt_des/demo_%d.mp4"%(i+1))

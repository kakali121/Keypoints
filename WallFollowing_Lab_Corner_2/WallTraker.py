...
LastEditTime : 2023-09-05 14:02:07
...
λ = 15                                            # The weight of smoothing the y ratio
...
class WallTraker:
    ...
    def _calculate_moving_average_y(self, new_y_ratio: float) -> float:
        ...
        if self.accumulated_y_ratio == 0: # If the accumulated y ratio is 0, set it to the current y ratio
            ...
            print("y_ratio_diff: ", y_ratio_diff)
            if y_ratio_diff > 5: # If the difference is too big, discard the current y ratio
    ...
    def chase_carrot(self)-> Tuple[int, float, int, bool, float, List[float]]:
        '''
        ...
        if num_matches <= MIN_NUM_MATCHES: return 0, 1, num_matches, True, 1, debug_dynamic_gain
        ...
        return x_diff, processed_y_ratio, num_matches, False, y_ratio, debug_dynamic_gain

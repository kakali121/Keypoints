...
def find_donkey(robot_descriptors, robot_keypoints, robot_frame, file=DES_FILE):
    ...
    robot_xy, donkey_xy = util.keypoint_coordinate(rob_ref_matches, robot_keypoints, ref_keypoints)
...
if __name__ == "__main__":
    ...
    ##### Carrot Persue ####################################################################################################
        ...
        # Find matching keypoints coordinates for retrived and reference keypoints
        robot_xy, carrot_xy = util.keypoint_coordinate(rob_goal_matches, robot_keypoints, goal_keypoints)
    ...
    ##### Control Analysis ####################################################################################################
        ...
        q_mx, q_my, q_rx, q_ry, t_mx, t_my, t_rx, t_ry = util.pair_analysis(robot_xy, carrot_xy)
        cx_diff = 0
        rx_ratio = 1
        cx_diff = (t_mx - q_mx)
        rx_ratio = t_rx / q_rx

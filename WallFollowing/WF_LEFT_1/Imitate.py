...
LastEditTime : 2023-09-12 19:13:21
FilePath     : /WF_LEFT_1/Imitate.py
...
MAX_HUMMING_DISTANCE = 40  # Max humming distance
MIN_NUM_MATCHES = 7  # Min number of matches
...
DRAW_TRACE = True  # Whether to draw the trace of the robot
...
myrobot = Robot.Robot(IP_ADDRESS, CONNECT_TO_ROBOT, DRAW_TRACE, SAVE_PATH)
...
out1 = cv2.VideoWriter(SAVE_PATH + "/robot.mp4", fourcc, fps, (400, 300))
out2 = cv2.VideoWriter(SAVE_PATH + "/carrot.mp4", fourcc, fps, (400, 300))
...
def plot_speeds():
    ...
    plt.ylabel("Values")
    plt.grid(True)
    plt.savefig(SAVE_PATH + "/ω_plot.pdf")
    ...
    plt.grid(True)
    plt.savefig(SAVE_PATH + "/num_match_plot.pdf")
    ...
    plt.grid(True)
    plt.savefig(SAVE_PATH + "/raw_ellipse_ratio_plot.pdf")
...
try:
    while streaming_video.isOpened():
        ...
        # if abs(x_diff) < 15 and abs(ω) < 40 and not lost:  # If the robot is close enough to the carrot

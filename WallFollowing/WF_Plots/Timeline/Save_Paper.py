"""
Author       : Hanqing Qi
Date         : 2023-09-10 22:27:27
LastEditors  : Hanqing Qi
LastEditTime : 2023-09-11 15:48:20
FilePath     : /WallFollowing_Lab_Corner_Plots/Timeline/save_paper.py
Description  : 
"""
import cv2
import numpy as np
...
if os.path.exists("../corner.mp4") == False:
    DESCRIPTOR_PATH = "side_demo_kpt_des_"
    VIDEO = "corner.mp4"
else:
    DESCRIPTOR_PATH = "../side_demo_kpt_des_"
    VIDEO = "../corner.mp4"
...
def analyze_kpt_des(frame, keypoints, descriptors, filename):
...
if __name__ == "__main__":
    ...
    for i in range(int(len(frames)/INTERVAL)):
        print("Interval", i+1)
        if os.path.exists(DESCRIPTOR_PATH + str(INTERVAL) + "_" + str(MAX_MATCH_DISTANCE)) == False:
            os.mkdir(DESCRIPTOR_PATH + str(INTERVAL) + "_" + str(MAX_MATCH_DISTANCE))
        temp_path = DESCRIPTOR_PATH + str(INTERVAL) + "_" + str(MAX_MATCH_DISTANCE) + "/side_demo_kpt_des%d.yml"%(i+1)
        analyze_kpt_des(frames, frame_kpt, frame_des, temp_path)

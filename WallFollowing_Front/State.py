...
LastEditTime : 2023-08-15 13:59:42
FilePath     : /WallFollowing_Front/State.py
...
MAX_MATCH_DISTANCE = 50  # The max humming distance between two descriptors
...
class State:
    def __init__(
        ...
        param       {*} self: -
        param       {np} frame: The frame of the state
        param       {bool} load: If true, load the keypoints and descriptors from the descriptor file
        ...
        self.keypoints = None  # The keypoints of the state
        self.descriptors = None  # The descriptors of the state
        if load:
            self._load_kpt_des(
                interval
            )  # Load the keypoints and descriptors from the descriptor file
        else:
            self._extract_kpt_des()  # Extract the keypoints and descriptors from the frame
    ...
    def get_match_coordinate(
        self, state: "State", filter: bool = True, draw_keypoints: bool = True
    ) -> Tuple[List[np.array], List[np.array]]:
        """
        ...
        if filter:
            matches = [
                match for match in matches if match.distance < MAX_MATCH_DISTANCE
            ]
    ...
    def _draw_keypoints_pair(
        self, query_coordinate: np.array, train_coordinate: np.array
    ) -> None:
        """
        ...
        if len(query_coordinate) != len(train_coordinate):
            raise ValueError(
                "The number of query coordinates must match the number of train coordinates."
            )
        ...
        for q_point, t_point in zip(query_coordinate, train_coordinate):
            ...
            self.temp_frame = cv2.circle(
                self.temp_frame, q_point, 4, (0, 255, 0), -1
            )  # Green for query
            self.temp_frame = cv2.circle(
                self.temp_frame, t_point, 4, (0, 0, 255), -1
            )  # Red for train

            # Draw arrow between matched keypoints
            self.temp_frame = cv2.arrowedLine(
                self.temp_frame, q_point, t_point, (255, 0, 0), 2, tipLength=0.2
            )  # Blue for arrows

    def compute_confidence_ellipse(
        self, query_coordinate: np.array, n_std=2.5, draw: bool = True
        ...
        if draw:
            ...
            self.temp_frame = cv2.circle(
                self.temp_frame, (mean_x, mean_y), 5, (255, 0, 255), -1
            )
